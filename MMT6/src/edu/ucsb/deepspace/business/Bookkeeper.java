...
public class Bookkeeper {
	...
	private boolean trkIni = false; //not yet implemented.  some check that prevents motion unless tracker is
									//initialized
	private boolean trkChecks = false;//same thing as trkIni except for health checks and startupchecks
	private TrackerMeasureMode trkMeasmode = TrackerMeasureMode.ADM;
	
	//bookkeeper is singleton.  there should be (this enforces it) only one instance of bookkeeper, ever.
	private Bookkeeper() {}
	...
	public void setReflectables(Map<String, Reflectable> reflectables) {
		...
		Display.getDefault().asyncExec(new Runnable() {
			public void run() {
				MMT6MainWindow.updateActuatorList(actuatorList);
				MMT6MainWindow.updateTargetList(targetList);
			}
		});
